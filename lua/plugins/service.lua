...
local bot = ngx.shared.bot -- Load bot shared dictonary
...
function plugin.onTextMessage(msg, blocks)
	...
	if blocks[1] == 'new_chat_member:bot' or blocks[1] == 'migrate_from_chat_id' then
		...
		if u.is_blocked_global(msg.from.id) then
			api.sendMessage(msg.chat.id, ("_You (user ID: %d) are in the blocked list_"):format(msg.from.id), true)
		...
		text = text .. ("I can do a lot of cool things. To discover about them, "
