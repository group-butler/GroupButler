...
function plugin.onTextMessage(msg, blocks)
	...
	if blocks[1] == 'link' then
		if msg.chat.username then
			...
			if not link then
				text = ("*No link* for this group. Ask the owner to save it with `/setlink [group link]`")
			else
				local title = msg.chat.title:escape_hard('link')
	...
	if blocks[1] == 'setlink' then
		if blocks[2] and blocks[2] == '-' then
			...
			if msg.chat.username then
				...
				text = ("The link has been set.\n*Here's the link*: %s"):format(substitution)
			else
				if not blocks[2] then
					text = ("This is not a *public supergroup*, so you need to write the link near `/setlink`")
				else
