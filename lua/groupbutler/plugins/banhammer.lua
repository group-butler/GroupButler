...
function _M:onTextMessage(blocks)
	...
	if blocks[1] == 'tempban' then
		...
		msg:send_reply(i18n:_("Use -/+ to edit the value, then select a timeframe to temporary ban the user"),
	...
	if blocks[1] == 'kick' then
		...
		if ok then
			...
			api:sendMessage(msg.chat.id, i18n:_("%s kicked %s!"):format(admin, kicked), 'html', true)
		else
	...
	if blocks[1] == 'ban' then
		...
		if ok then
			...
			api:sendMessage(msg.chat.id, i18n:_("%s banned %s!"):format(admin, kicked), 'html', true)
		else
			msg:send_reply(err, "Markdown")
		end
	end
	if blocks[1] == 'fwdban' then
		if not msg.reply or not msg.reply.forward_from then
			msg:send_reply(i18n:_("_Use this command in reply to a forwarded message_"), "Markdown")
		else
			user_id = msg.reply.forward_from.id
			local ok, err = u:banUser(chat_id, user_id)
			if ok then
				u:logEvent('ban', msg, {motivation = get_motivation(msg), admin = admin, user = kicked, user_id = user_id})
				api:sendMessage(msg.chat.id, i18n:_("%s banned %s!"):format(admin, u:getname_final(msg.reply.forward_from)), 'html')
			else
				msg:send_reply(err, "Markdown")
			end
		end
	...
	if blocks[1] == 'unban' then
		if u:is_admin(chat_id, user_id) then
			msg:send_reply(i18n:_("_An admin can't be unbanned_"), "Markdown")
			return
		end
		...
		if ok.status ~= 'kicked' then
			msg:send_reply(i18n:_("This user is not banned!"))
			return
		end
		...
		msg:send_reply(i18n:_("%s unbanned by %s!"):format(kicked, admin), 'html')
	end
end
...
function _M:onCallbackQuery(matches)
	...
	if not u:can(msg.chat.id, msg.from.id, 'can_restrict_members') then
		api:answerCallbackQuery(msg.cb_id, i18n:_("You don't have the permissions to restrict members"), true)
	else
		if matches[1] == 'nil' then
			api:answerCallbackQuery(msg.cb_id,
				i18n:_("Tap on the -/+ buttons to change this value. Then select a timeframe to execute the ban"), true)
		elseif matches[1] == 'val' then
			...
			if matches[2] == 'm' then
				...
				if new_value < 1 then
					api:answerCallbackQuery(msg.cb_id, i18n:_("You can't set a lower value"))
			...
			elseif matches[2] == 'p' then
				...
				if new_value > 100 then
					api:answerCallbackQuery(msg.cb_id, i18n:_("Stop!!!"), true)
					return --don't proceed
				else
					red:setex(key, 3600, new_value)
				end
