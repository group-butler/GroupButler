...
function _M:onTextMessage(blocks)
	...
	if blocks[1] == 'new_chat_member:bot' or blocks[1] == 'migrate_from_chat_id' then
		-- set the language
		--[[locale.language = red:get(string.format('lang:%d', msg.from.user.id)) or config.lang
		...
		--[[if locale.language then
			red:set(string.format('lang:%d', msg.from.chat.id), locale.language)
		end]]
		...
		--[[if not u:is_admin(msg.from.chat.id, bot.id) then
			if u:is_owner(msg.from.chat.id, msg.from.user.id) then
