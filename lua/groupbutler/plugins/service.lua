...
function _M:onTextMessage(blocks)
	...
	if blocks[1] == 'new_chat_member:bot' or blocks[1] == 'migrate_from_chat_id' then
		if u:is_blocked_global(msg.from.id) then
			api:sendMessage(msg.chat.id, i18n:_("_You (user ID: %d) are in the blocked list_"):format(msg.from.id), "Markdown")
			api:leaveChat(msg.chat.id)
			return
		end
		if config.bot_settings.admin_mode and not u:is_superadmin(msg.from.id) then
			api:sendMessage(msg.chat.id, i18n:_("_Admin mode is on: only the bot admin can add me to a new group_"), "Markdown")
			api:leaveChat(msg.chat.id)
			return
		end
		...
		text = text .. i18n:_("I can do a lot of cool things. To discover about them, "
