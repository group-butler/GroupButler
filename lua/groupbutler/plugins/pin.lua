...
local function pin_message(self, chat_id, message_id)
	self.db:set("chat:"..chat_id..":pin", message_id)
	return api.pin_chat_message(chat_id, message_id, self.u:is_silentmode_on(chat_id))
end
...
local function edit_pin(self, msg, pin_text)
	local pin_id = self.db:get("chat:"..msg.chat.id..":pin")
	if pin_id == null then
...
local function last_pin(self, msg)
	...
	if not ok and err.description:lower():match("reply message not found") then
		...
		self.db:del("chat:"..msg.chat.id..":pin")
		return
	end
end
