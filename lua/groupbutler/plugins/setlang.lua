...
function _M:onTextMessage()
	...
	if msg.from.chat.type == 'private' or (msg.from.chat.id < 0 and u:is_allowed('config', msg.from.chat.id, msg.from.user)) then
...
function _M:onCallbackQuery(blocks)
	...
	if msg.from.chat.type ~= 'private' and not msg.from:isAdmin() then
		...
		if blocks[1] == 'selectlang' then
			...
			if (blocks[1] == 'ar_SA' or blocks[1] == 'fa_IR') and msg.from.chat.type ~= 'private' then
			...
			api:editMessageText(msg.from.chat.id, msg.message_id, nil, i18n("English language is *set*") ..
i18n([[.
