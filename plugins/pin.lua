    ...
    if blocks[1] == 'pin' then
		if roles.is_admin_cached(msg) then
			...
			if not res then
	    		...
	    		api.sendMessage(msg.chat.id, _("You can now pin this message and use `/editpin [new text]` to edit it, without send the new message to pin again"), true, nil, res.result.message_id)
	    	end
    	end
	end
