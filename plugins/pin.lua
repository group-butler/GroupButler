...
function plugin.onTextMessage(msg, blocks)
	...
	if u.is_allowed('texts', msg.chat.id, msg.from) then
		if not blocks[2] then
			...
			if pin_id then
			    api.sendMessage(msg.chat.id, ('Last message generated by `/pin` ^'), true, nil, pin_id)
			end
			return
		end
		...
		if pin_id then --try to edit the old message
			...
			if not res then
	    		...
	    		api.sendMessage(msg.chat.id, ("Message edited. Check it here"), nil, nil, pin_id)
	    	end
		end
		if not pin_id then
			local reply_markup, new_text = u.reply_markup_from_text(blocks[2])
			...
			if not res then
    			...
    			local text
    			if was_deleted then
    				text = ("The old message generated with `/pin` does not exist anymore, so I can't edit it. This is the new message that can be now pinned")
    			else
    				text = ("This message can now be pinned. Use `/pin [new text]` to edit it without having to send it again")
    			end
